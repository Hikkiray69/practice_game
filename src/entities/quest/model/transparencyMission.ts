import { QuestScenario } from "./types";

export const transparencyMission: QuestScenario = {
  id: "mission-transparency-2",
  valueTag: "transparency",
  title: "Прозрачность: честный статус проекта",
  intro: "Риск по внешней интеграции: клиент ждёт статус. Сначала диалог с коллегой, потом — как сообщать о ситуации.",
  preamble: [
    {
      speaker: "npc",
      text: "Клиент ждёт «всё зелёное», а по интеграции уже пахнет риском. Я не знаю, что ему писать, чтобы не соврать и не паниковать зря.",
    },
    {
      speaker: "player",
      text: "Понял. Не оставляй людей в неведении — давай честно соберём картину, я с тобой, разложим по полочкам.",
    },
    {
      speaker: "npc",
      text: "Отлично. Тогда решаем: на время приглушить новости или открыто обозначить риск и наш план смягчения. Выбирай.",
    },
  ],
  minigameId: "memory",
  choices: [
    {
      id: "hide-risk",
      label: "Скрыть риск до финальной проверки",
      consequenceId: "hide-risk-outcome",
    },
    {
      id: "open-risk",
      label: "Открыто обозначить риск и mitigation-план",
      consequenceId: "open-risk-outcome",
    },
  ],
  consequences: [
    {
      id: "hide-risk-outcome",
      scoreDelta: 4,
      qualityDelta: -4,
      speedDelta: 3,
      summary: "Краткосрочно спокойнее, но доверие падает и риск срыва растет.",
    },
    {
      id: "open-risk-outcome",
      scoreDelta: 9,
      qualityDelta: 5,
      speedDelta: -1,
      summary: "Риск прозрачен, команда синхронизирована, доверие клиента выше.",
    },
  ],
};
